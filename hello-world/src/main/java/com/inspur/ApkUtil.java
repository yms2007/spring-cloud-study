...
public class ApkUtil {
    ...
    private String getAaptPath(){
        if(mAaptPath == null){
                mAaptPath = "F:\\ideaWorkspace\\spring-cloud-study\\hello-world\\src\\main\\java\\aapt.exe";//windows环境下直接指向appt.exe
        }
        return mAaptPath;
    }
    ...
    public ApkInfo getApkInfo(String apkPath) throws Exception {
        ...
        //通过命令调用aapt工具解析apk文件
        Process process = mBuilder.command(getAaptPath(), "d", "badging", apkPath)
                .start();
        InputStream is = null;
...
//          System.out.println(source);
  ...
  //  }
    ...
    public static void main(String[] args) {
        try {
            //String demo = "d:/a.apk";
            File[] files = (new File("G:\\工作文档\\170904应用商城\\1-26\\temp")).listFiles();
            for(File f:files){
                String path = f.getPath();
                ApkInfo apkInfo = new ApkUtil().getApkInfo(path);
                System.out.println(apkInfo.getApplicationLable()+"\t" +apkInfo.getPackageName());
            }
